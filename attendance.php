	...
	if(isset($_POST['employee'])){
		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
					$output['error'] = true;
					$output['message'] = 'Bạn đã chấm công lúc vào rồi.';
				}
				else{
					//updates
				...
				if($query->num_rows < 1){
					$output['error'] = true;
					$output['message'] = 'Không thể thêm giờ vào và giờ ra.';
				}
				else{
					$row = $query->fetch_assoc();
					if($row['time_out'] != '00:00:00'){
						$output['error'] = true;
						$output['message'] = 'Bạn đã chấm công lúc về rồi.';
					}
					else{
			...
			$output['error'] = true;
			$output['message'] = 'Không thể tìm thấy mã nhân viên.';
		}
